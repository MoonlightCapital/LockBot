class Antispam {
  ...
  function constructor(bot, options) {
...
module.exports = function (bot, options) {
  // Set options
  this.warnBuffer = (options && options.warnBuffer) || 3
  this.maxBuffer = (options && options.maxBuffer) || 5
  this.interval = (options && options.interval) || 1000
  this.warningMessage = (options && options.warningMessage) || "stop spamming or I'll whack your head off."
  this.banMessage = (options && options.banMessage) || "has been banned for spamming, anyone else?"
  this.maxDuplicatesWarning = (options && options. maxDuplicatesWarning || 7)
  this.maxDuplicatesBan = (options && options. maxDuplicatesBan || 10)
  this.deleteMessagesAfterBanForPastDays = (options && options.deleteMessagesAfterBanForPastDays || 7)
  this.exemptRoles = (options && options.exemptRoles) || []
  this.exemptUsers = (options && options.exemptUsers) || []
   ...
   */
  function warn(msg, bot) {
    warned.push(msg.author.id)

    if(settings.antispam.alert && settings.antispam.alert_channel) {
